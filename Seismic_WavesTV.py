...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 16 09:33:50 2024

@author: keiragupta
"""
"""
SEISMIC WAVES (TAYLOR'S VERSION)

This code plots how the duration of seismic waves change over a set distance when the ground materials are changing (changing
ground material = changing velocity). Using the seismic waves generated by Taylor Swift's "The Eras Tour", this code models
the 1D advection of the seismic waves through the ground, given that the ground materials are changing. The wave velocity
in the model gradually decreases, and the final figure shows that the waves take gradually longer to travel over time.
...
* P-wave velocities for Limestone, Sandstone, Clay, and Sand were drawn from this source.
...
setlist_array = df_setlist.to_numpy() # converting setlist data into numpy array

# Importing ground condition/material data: contains P-wave velocity for 4 types of sedimentary ground material
df_gc = pd.read_csv('Groundconditions.csv', skiprows = 1)
gc_array = df_gc.to_numpy() # converting ground condition data into numpy array
...
# The velocity decreases every 200km
# P-wave velocity data is found in the ground conditions dataset
# Each P-wave velocity value represents that of a sedimentary unit 

velocity = np.zeros(nodes) # set up empty array to be filled
velocity[x < 200000] = gc_array[0, 1] # m/s - Limestone (6000 m/s)
velocity[(x >= 200000) & (x < 400000)] = gc_array[1, 1] # m/s - Sandstone (2900 m/s)
velocity[(x >= 400000) & (x < 600000)] = gc_array[2, 1] # m/s - Clay (1800 m/s)
velocity[x >= 600000] = gc_array[3, 1] # m/s - Sand (1500 m/s)
...
plt.imshow(concentration_over_time.T, aspect = 'auto', cmap = 'magma', origin='lower', extent = [0, 1000, 0, 180]) 
...
plt.ylabel('Time (min)')
plt.title('The Eras Tour Seismic Intensity Profile: Through Time and over Changing Ground Materials')
plt.legend()
plt.show() 